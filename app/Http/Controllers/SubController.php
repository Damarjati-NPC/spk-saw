...
class SubController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (DB::table('subs')->where('kode_id', $request->kode)->sum('bobot') + $request->bobot >= 10.0) {
            return redirect('sub/' . $request->kode)->with('delete', 'Total bobot lebih dari 10 !!');
        }
    ...
    public function update(Request $request, Sub $sub)
    {
        ...
        $sum = Sub::where('id', '!=', $sub->id)
            ->where('kode_id', $sub->kode_id)
            ->sum('bobot');

        if ($sum + $request->bobot > 10) {
            return redirect('sub/' . $request->kode)->with('delete', 'Total bobot lebih dari 10 !!');
        }
